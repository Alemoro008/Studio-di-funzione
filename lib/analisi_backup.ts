...
export function calcolaDerivataSeconda(
  ...
  try {
    ...
    // Prova a semplificare ulteriormente
    risultato =
      simplify(
        risultato,
        {
    ...
    // Riduce espressioni equivalenti come:
    // 2*(x^2 + (x-1)^2) - 4*x*(x-1)
    // ---------------------------------
    //             (x-1)^3
    //
    ...
    try {
      ...
      // Mantiene comunque il risultato precedente
    }
...
export function calcolaAsintotiObliqui(
  ...
  try {
    function calcola(
       ...
       * m = (f(x1) - f(x2)) / (x1 - x2)
       *
       * È più preciso di f(x) / x
       * per calcolare l'asintoto obliquo.
       */
      const m =
        (y1 - y2) /
        (x1 - x2);
  ...
  numeroPulito(
    ...
    if (positivo !== null) {
      risultati.push(positivo);
    }

    if (negativo !== null) {
      risultati.push(negativo);
    }
